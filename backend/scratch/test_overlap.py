...
def calculate_similarity_original(item_desc, task_name):
...
def calculate_similarity_overlap(item_desc, task_name):
    item_tokens = get_word_tokens(item_desc)
    task_tokens = get_word_tokens(task_name)
    if not item_tokens or not task_tokens:
        return 0
    ...
    # Normalize by min of tokens length (Overlap Coefficient style)
    min_len = min(len(item_tokens), len(task_tokens))
    return match_score / min_len
...
def main():
    ...
    for i in range(10, min(304, budget_df.shape[0])):
    ...
    print("\n--- COMPARANDO ALGORITMO ORIGINAL VS. COEFICIENTE DE TRASLAPE (OVERLAP) ---")
    for target_desc in unmapped_examples:
        b_item = next((b for b in budget_items if target_desc.lower() in b["desc"].lower()), None)
        if not b_item: continue
        print(f"\nPresupuesto: '{b_item['desc']}'")
        
        # Original
        scored_orig = []
        for task in granular_tasks:
            score = calculate_similarity_original(b_item["desc"], task["name"])
            scored_orig.append((task["name"], score))
        scored_orig.sort(key=lambda x: -x[1])
        
        # Overlap
        scored_over = []
        for task in granular_tasks:
            score = calculate_similarity_overlap(b_item["desc"], task["name"])
            scored_over.append((task["name"], score))
        scored_over.sort(key=lambda x: -x[1])
        
        print("  [ORIGINAL] Top 3:")
        for t_name, score in scored_orig[:3]:
            print(f"    {score:.2f} -> '{t_name}'")
            
        print("  [OVERLAP ] Top 3:")
        for t_name, score in scored_over[:3]:
            print(f"    {score:.2f} -> '{t_name}'")
