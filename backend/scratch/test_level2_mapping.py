...
def get_top_budget_candidates(task_name, budgets, count=12):
    ...
    return [x["budget"] for x in scored[:count] if x["score"] > 0]
...
def main():
    ...
    # Include level >= 2
    granular_tasks = [t for t in tasks if t["level"] >= 2]
    print(f"Total tasks with level >= 2: {len(granular_tasks)}")
    ...
    print(f"Total Direct Budget in Excel: $ {total_direct_budget:,.2f} COP")

    # 1. Theoretical Max if using Top 1 of each task
    top1_assigned_codes = set()
    for task in granular_tasks:
        cands = get_top_budget_candidates(task["name"], budget_items, 1)
        if cands:
            top1_assigned_codes.add(cands[0]["code"])
            
    top1_assigned_value = sum(b["val"] for b in budget_items if b["code"] in top1_assigned_codes)
    top1_percentage = (top1_assigned_value / total_direct_budget) * 100
    print(f"\n--- TEÓRICO SI USAMOS TOP 1 DIRECTAMENTE CON NIVEL >= 2 ---")
    print(f"Presupuesto Mapeado Directamente: $ {top1_assigned_value:,.2f} COP")
    print(f"Porcentaje: {top1_percentage:.2f}% del costo directo")
    print(f"Ítems de presupuesto no asignados (Huérfanos): {len(budget_items) - len(top1_assigned_codes)} de {len(budget_items)}")

    # 2. Theoretical Max if using Top 12 candidates of each task (and perfect selection)
    top12_accessible_codes = set()
    for task in granular_tasks:
        cands = get_top_budget_candidates(task["name"], budget_items, 12)
        for c in cands:
            top12_accessible_codes.add(c["code"])
            
    top12_assigned_value = sum(b["val"] for b in budget_items if b["code"] in top12_accessible_codes)
    top12_percentage = (top12_assigned_value / total_direct_budget) * 100
    print(f"\n--- MÁXIMO TEÓRICO ACCESIBLE EN EL TOP 12 CON NIVEL >= 2 ---")
    print(f"Presupuesto máximo accesible: $ {top12_assigned_value:,.2f} COP")
    print(f"Porcentaje máximo accesible: {top12_percentage:.2f}% del costo directo")
    print(f"Ítems de presupuesto teóricamente inalcanzables: {len(budget_items) - len(top12_accessible_codes)} de {len(budget_items)}")

    # Print some of the theoretical orphans in Top 1
    unmapped_in_top1 = [b for b in budget_items if b["code"] not in top1_assigned_codes]
    unmapped_in_top1.sort(key=lambda x: -x["val"])
    print(f"\nTop 10 ítems más caros huérfanos en el MODO LOCAL (Top 1) con nivel >= 2:")
    for b in unmapped_in_top1[:10]:
        print(f"  $ {b['val']:,.2f} COP -> Code: {b['code']} | Chapter: {b['chapter']} | Desc: '{b['desc']}'")
