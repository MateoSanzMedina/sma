...
def main():
    ...
    print("\n--- ANALIZANDO LOS TOP CANDIDATOS SUGERIDOS PARA CADA TAREA DEL CRONOGRAMA ---")
    for t_name in test_tasks:
        print(f"\nTarea del Cronograma: '{t_name}'")
        scored_budgets = []
        for b in budget_items:
            score = calculate_similarity_original(b["desc"], t_name)
            scored_budgets.append((b, score))
        scored_budgets.sort(key=lambda x: (-x[1], x[0]["desc"]))
        
        print("Top 5 candidatos de presupuesto sugeridos (Original):")
