...
def calculate_similarity_with_chapter(item_desc, item_chapter, task_name):
    ...
    # Match against description (weight: 1.0)
    desc_score = 0
    if desc_tokens:
        for i_tok in desc_tokens:
            ...
            desc_score += best_token_score
        desc_score = desc_score / min(len(desc_tokens), len(task_tokens))
        
    # Match against chapter (weight: 0.5)
    chapter_score = 0
    if chapter_tokens:
        for c_tok in chapter_tokens:
            ...
            for t_tok in task_tokens:
                ...
                if current_score > best_token_score:
                    best_token_score = current_score
            chapter_score += best_token_score
        chapter_score = chapter_score / min(len(chapter_tokens), len(task_tokens))
...
def main():
    ...
    test_tasks = [
        "construcción de transfo y cableado",
        "domiciliares electricas teleco y pedestales",
        "tuberia electrica",
        "cajas electricas"
    ]
    
    print("\n--- ANALIZANDO LOS TOP CANDIDATOS CON LA NUEVA SIMILITUD DE TRASLAPE + CAPÍTULO ---")
    for t_name in test_tasks:
        print(f"\nTarea del Cronograma: '{t_name}'")
        scored_budgets = []
        for b in budget_items:
            score = calculate_similarity_with_chapter(b["desc"], b["chapter"], t_name)
            scored_budgets.append((b, score))
        scored_budgets.sort(key=lambda x: (-x[1], x[0]["desc"]))
        
        print("Top 5 candidatos sugeridos (NUEVO ALGORITMO):")
        for b, score in scored_budgets[:5]:
            print(f"  Score {score:.2f} -> Code: {b['code']} | Desc: '{b['desc']}' (Cap: {b['chapter']})")
