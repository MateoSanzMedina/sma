...
def main():
    ...
    unreachable_items = [b for b in budget_items if b["code"] not in top12_accessible_codes]
    unreachable_items.sort(key=lambda x: -x["val"])
    
    print(f"\n--- TOTAL UNREACHABLE ITEMS IN TOP 12: {len(unreachable_items)} of {len(budget_items)} ---")
    print(f"Total Unreachable Value: $ {sum(b['val'] for b in unreachable_items):,.2f} COP")
    
    print("\nTop 25 most expensive completely unreachable items:")
    for idx, b in enumerate(unreachable_items[:25]):
        print(f"{idx+1}. Code: {b['code']} | Chapter: {b['chapter']} | Val: $ {b['val']:,.2f} COP | Desc: '{b['desc']}'")
